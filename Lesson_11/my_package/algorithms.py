# Create by Bender
"""
Данный модуль позволяет работать с несколькими алгоритмами числовых
последовательностей.
Алгоритм "without_00", позволяет узнать количество комбинацый из нулей и едениц,
в которых нет двух нулей рядом.
Алгоритм "reverse_it", позволяет развернуть число задом на перёд, не используя
списков строк и пр..
Алгоритм "count_first", позволяет вывести первые n членов монотонной
последовательности,
в которой каждое натуральное число k встречается ровно k раз,
например: 10 => 1, 2, 2, 3, 3, 3, 4, 4, 4, 4,
"""
...
def without_00(zero: int, one: int) -> int:
    """
    Функция позволяет узнать количество комбинацый из нулей и едениц,
    в которых нет двух нулей рядом.
...
def count_first(user_num: int, nums="", i=1, j=1) -> str:
    """
    Функция позволяет позволяет вывести первые n членов монотонной
    последовательности
    :param user_num: целочисленное значение, которое вводит пользователь
    :param nums:     строка, в которую аккумулируются получаемые значения,
    в ходе работы функции
    :param i:        счётчик циклов, по умолчанию равен еденице
    :param j:        получаемое число в ходе работы одного цикла,
    по умолчанию равное еденице
    :return:         возвращает результат строку со значением,
